...
```
## 5.3 使用ModelAndView实现转发


将处理请求的方法的返回值，改为`ModelAndView`类型，
...
>其实转发操作也有前缀`forward:error`;

## 转发与重定向的区别
- 转发发生在服务器内部，客户端只发出了1次请求！整个过程中，浏览器地址栏中的URL是不变的。
- 重定向是客户端第一次向服务器发送请求后，服务器给出302响应码及重定向的目标路径。客户端收到30响应码后，会自动发送第二次请求，URL会改变。服务器给302响应码时，必须指定重定向的路径。HTTP协议是无状态协议，所以，服务器在2次处理过程中的数据不能直接共享使用。
